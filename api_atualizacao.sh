...
# 4. Alterar permissões do diretório /api-integration
echo "Alterando permissões do diretório /api-integration..."
sudo chmod 777 -R /api-integration
if [ $? -ne 0 ]; then
  echo "Erro ao alterar permissões do diretório /api-integration."
  exit 1
fi

# 5. Executar o script de atualização
echo "Executando o script upgrade-unix.sh..."
./upgrade-unix.sh
if [ $? -ne 0 ]; then
  echo "Erro ao executar o script upgrade-unix.sh."
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "Erro ao reiniciar o serviço api-gestaoparts-notify."
  exit 1
fi
